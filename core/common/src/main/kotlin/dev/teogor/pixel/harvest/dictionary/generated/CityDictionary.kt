package dev.teogor.pixel.harvest.dictionary.generated

import dev.teogor.pixel.harvest.dictionary.Dictionary

private val citiesSet = setOf(
    "abu ghraib, iraq",
    "abuja, nigeria",
    "accra, ghana",
    "adana, turkey",
    "adelaide, australia",
    "ahmedabad, india",
    "alba iulia, romania",
    "alexandria, egypt",
    "alexandria, romania",
    "amsterdam, netherlands",
    "anhui, china",
    "ankara, turkey",
    "antalya, turkey",
    "antipolo, philippines",
    "antofagasta, chile",
    "arad, romania",
    "as suwayq, oman",
    "ashiaman, ghana",
    "asyut, egypt",
    "athens, greece",
    "auckland, new zealand",
    "bacău, romania",
    "baghdad, iraq",
    "baia mare, romania",
    "baicoi, romania",
    "bandung, indonesia",
    "bangalore, india",
    "bangkok, thailand",
    "banja luka, bosnia and herzegovina",
    "bar, montenegro",
    "barcelona, spain",
    "basel, switzerland",
    "basra, iraq",
    "bawshar, oman",
    "beijing, china",
    "bekasi, indonesia",
    "belgrade, serbia",
    "belo horizonte, brazil",
    "benin city, nigeria",
    "bergen, norway",
    "berlin, germany",
    "bern, switzerland",
    "bien hoa, vietnam",
    "bihać, bosnia and herzegovina",
    "bijelo polje, montenegro",
    "bitola, north macedonia",
    "bizerte, tunisia",
    "biên hòa, vietnam",
    "botoșani, romania",
    "brasília, brazil",
    "brașov, romania",
    "brisbane, australia",
    "brussels, belgium",
    "brăila, romania",
    "brčko, bosnia and herzegovina",
    "bucharest, romania",
    "budapest, hungary",
    "budva, montenegro",
    "buenos aires, argentina",
    "bugojno, bosnia and herzegovina",
    "bukavu, democratic republic of the congo",
    "burgas, bulgaria",
    "bursa, turkey",
    "busan, south korea",
    "buzău, romania",
    "cairo, egypt",
    "caloocan, philippines",
    "can tho, vietnam",
    "canberra, australia",
    "cape coast, ghana",
    "carei, romania",
    "cebu city, philippines",
    "celje, slovenia",
    "cetinje, montenegro",
    "chengdu, china",
    "chiang mai, thailand",
    "chongqing, china",
    "christchurch, new zealand",
    "cluj-napoca, romania",
    "codlea, romania",
    "cologne, germany",
    "constanța, romania",
    "craiova, romania",
    "curitiba, brazil",
    "câmpina, romania",
    "córdoba, argentina",
    "călărași, romania",
    "da nang, vietnam",
    "daegu, south korea",
    "davao city, philippines",
    "debrecen, hungary",
    "delhi, india",
    "depok, indonesia",
    "deva, romania",
    "dhaka, bangladesh",
    "diyarbakır, turkey",
    "dongguan, china",
    "dongying, china",
    "dorohoi, romania",
    "dortmund, germany",
    "drammen, norway",
    "drobeta-turnu severin, romania",
    "dublin, ireland",
    "dunedin, new zealand",
    "düsseldorf, germany",
    "el-mahalla el-kubra, egypt",
    "erbil, iraq",
    "erzurum, turkey",
    "eskişehir, turkey",
    "espoo, finland",
    "essen, germany",
    "ettadhamen, tunisia",
    "faisalabad, pakistan",
    "focșani, romania",
    "fortaleza, brazil",
    "foshan, china",
    "frankfurt, germany",
    "fredrikstad, norway",
    "fukuoka, japan",
    "gabès, tunisia",
    "gafsa, tunisia",
    "galați, romania",
    "gaziantep, turkey",
    "geneva, switzerland",
    "giurgiu, romania",
    "giza, egypt",
    "gjakova, kosovo",
    "gold coast, australia",
    "gostivar, north macedonia",
    "gothenburg, sweden",
    "graz, austria",
    "guangzhou, china",
    "gujranwala, pakistan",
    "győr, hungary",
    "haiphong, vietnam",
    "hamburg, germany",
    "hamilton, new zealand",
    "hangzhou, china",
    "hanoi, vietnam",
    "hat yai, thailand",
    "helsinki, finland",
    "heraklion, greece",
    "herceg novi, montenegro",
    "hiroshima, japan",
    "ho chi minh city, vietnam",
    "hong kong, china",
    "huizhou, china",
    "hunedoara, romania",
    "huế, vietnam",
    "hyderabad, india",
    "hyderabad, pakistan",
    "iași, romania",
    "ibadan, nigeria",
    "ibri, oman",
    "incheon, south korea",
    "innsbruck, austria",
    "islamabad, pakistan",
    "istanbul, turkey",
    "izmir, turkey",
    "jakarta, indonesia",
    "jiangmen, china",
    "jinan, china",
    "jining, china",
    "kaduna, nigeria",
    "kairouan, tunisia",
    "kananga, democratic republic of the congo",
    "kano, nigeria",
    "kaohsiung, taiwan",
    "karachi, pakistan",
    "karlovac, croatia",
    "kayseri, turkey",
    "kecskemét, hungary",
    "khon kaen, thailand",
    "kinshasa, democratic republic of the congo",
    "kirkuk, iraq",
    "kisangani, democratic republic of the congo",
    "klagenfurt, austria",
    "kolkata, india",
    "kolwezi, democratic republic of the congo",
    "konya, turkey",
    "koper, slovenia",
    "kragujevac, serbia",
    "kranj, slovenia",
    "kristiansand, norway",
    "kuala lumpur, malaysia",
    "kumanovo, north macedonia",
    "kumasi, ghana",
    "kuopio, finland",
    "kyoto, japan",
    "la plata, argentina",
    "lagos, nigeria",
    "lahore, pakistan",
    "lahti, finland",
    "larissa, greece",
    "lausanne, switzerland",
    "leskovac, serbia",
    "lima, peru",
    "linköping, sweden",
    "linz, austria",
    "lisbon, portugal",
    "ljubljana, slovenia",
    "london, united kingdom",
    "los angeles, united states",
    "lubumbashi, democratic republic of the congo",
    "lucerne, switzerland",
    "lugoj, romania",
    "lupeni, romania",
    "luxor, egypt",
    "macau, china",
    "madrid, spain",
    "maiduguri, nigeria",
    "makassar, indonesia",
    "malatya, turkey",
    "malmö, sweden",
    "manaus, brazil",
    "mandalay, myanmar",
    "mangalia, romania",
    "manila, philippines",
    "mansoura, egypt",
    "manukau, new zealand",
    "mar del plata, argentina",
    "maribor, slovenia",
    "mbuji-mayi, democratic republic of the congo",
    "medan, indonesia",
    "mediaș, romania",
    "melbourne, australia",
    "mendoza, argentina",
    "mersin, turkey",
    "mexico city, mexico",
    "milan, italy",
    "mioveni, romania",
    "miskolc, hungary",
    "moscow, russia",
    "mostar, bosnia and herzegovina",
    "mosul, iraq",
    "multan, pakistan",
    "mumbai, india",
    "munich, germany",
    "muscat, oman",
    "nagoya, japan",
    "nairobi, kenya",
    "najaf, iraq",
    "nakhon ratchasima, thailand",
    "nakhon si thammarat, thailand",
    "nanjing, china",
    "new york city, united states",
    "newcastle, australia",
    "nha trang, vietnam",
    "nikšić, montenegro",
    "niš, serbia",
    "nonthaburi, thailand",
    "north shore, new zealand",
    "novi sad, serbia",
    "novo mesto, slovenia",
    "nyíregyháza, hungary",
    "obuasi, ghana",
    "ohrid, north macedonia",
    "oradea, romania",
    "osaka, japan",
    "osijek, croatia",
    "oslo, norway",
    "oulu, finland",
    "pak kret, thailand",
    "paris, france",
    "pasig, philippines",
    "patras, greece",
    "penang, malaysia",
    "perth, australia",
    "peshawar, pakistan",
    "petrila, romania",
    "peć, kosovo",
    "phnom penh, cambodia",
    "piatra neamț, romania",
    "piraeus, greece",
    "pitești, romania",
    "pleven, bulgaria",
    "pljevlja, montenegro",
    "ploiești, romania",
    "plovdiv, bulgaria",
    "podgorica, montenegro",
    "port harcourt, nigeria",
    "port said, egypt",
    "prague, czech republic",
    "prilep, north macedonia",
    "pristina, kosovo",
    "prizren, kosovo",
    "ptuj, slovenia",
    "puente alto, chile",
    "pula, croatia",
    "pécs, hungary",
    "qingdao, china",
    "quezon city, philippines",
    "quilmes, argentina",
    "rawalpindi, pakistan",
    "recife, brazil",
    "reykjavik, iceland",
    "rhodes, greece",
    "rijeka, croatia",
    "rio de janeiro, brazil",
    "riyadh, saudi arabia",
    "rome, italy",
    "rosario, argentina",
    "ruse, bulgaria",
    "râmnicu sărat, romania",
    "râmnicu vâlcea, romania",
    "saham, oman",
    "salalah, oman",
    "salvador, brazil",
    "salzburg, austria",
    "san bernardo, chile",
    "san miguel de tucumán, argentina",
    "sandnes, norway",
    "santiago, chile",
    "sapporo, japan",
    "sarajevo, bosnia and herzegovina",
    "satu mare, romania",
    "seeb, oman",
    "sekondi-takoradi, ghana",
    "semarang, indonesia",
    "sendai, japan",
    "seoul, south korea",
    "sesvete, croatia",
    "sfax, tunisia",
    "shanghai, china",
    "shenzhen, china",
    "shijiazhuang, china",
    "shubra el-kheima, egypt",
    "sibiu, romania",
    "siem reap, cambodia",
    "sighetu marmației, romania",
    "singapore city, singapore",
    "singapore, singapore",
    "sivas, turkey",
    "skopje, north macedonia",
    "slatina, romania",
    "sliven, bulgaria",
    "sofia, bulgaria",
    "sohar, oman",
    "sollentuna, sweden",
    "sousse, tunisia",
    "split, croatia",
    "st. gallen, switzerland",
    "stara zagora, bulgaria",
    "stavanger, norway",
    "stockholm, sweden",
    "stuttgart, germany",
    "subotica, serbia",
    "suceava, romania",
    "suez, egypt",
    "sulaymaniyah, iraq",
    "sunyani, ghana",
    "surabaya, indonesia",
    "sydney, australia",
    "szeged, hungary",
    "são paulo, brazil",
    "taguig, philippines",
    "tai'an, china",
    "taipei, taiwan",
    "talcahuano, chile",
    "tamale, ghana",
    "tampere, finland",
    "tangerang, indonesia",
    "temuco, chile",
    "tetovo, north macedonia",
    "thessaloniki, greece",
    "tianjin, china",
    "timișoara, romania",
    "tokyo, japan",
    "trabzon, turkey",
    "trondheim, norway",
    "tshikapa, democratic republic of the congo",
    "tunis, tunisia",
    "turku, finland",
    "tuzla, bosnia and herzegovina",
    "târgoviște, romania",
    "târgu jiu, romania",
    "târgu mureș, romania",
    "udon thani, thailand",
    "ulsan, south korea",
    "uppsala, sweden",
    "valparaíso, chile",
    "vantaa, finland",
    "varna, bulgaria",
    "vaslui, romania",
    "velenje, slovenia",
    "vienna, austria",
    "vientiane, laos",
    "villach, austria",
    "viña del mar, chile",
    "volos, greece",
    "västerås, sweden",
    "waitakere, new zealand",
    "warsaw, poland",
    "weifang, china",
    "wellington, new zealand",
    "wels, austria",
    "winterthur, switzerland",
    "wuhan, china",
    "xi'an, china",
    "yangon, myanmar",
    "yantai, china",
    "yokohama, japan",
    "zadar, croatia",
    "zagreb, croatia",
    "zalău, romania",
    "zamboanga city, philippines",
    "zaria, nigeria",
    "zemun, serbia",
    "zenica, bosnia and herzegovina",
    "zhongshan, china",
    "zhuhai, china",
    "zibo, china",
    "zurich, switzerland",
    "örebro, sweden",
    "štip, north macedonia",
)

class CityDictionary(list: Set<String>) : Dictionary(list) {
    class CityDictionaryBuilder : Builder()

    enum class CityTypes(private val citySet: Set<String>) : Type {
        CITIES(citiesSet),
        ALL(citiesSet);

        override fun getSet(): Set<String> {
            return citySet
        }
    }

    companion object {
        fun builder(block: CityDictionaryBuilder.() -> Unit): CityDictionary {
            val builder = CityDictionaryBuilder()
            builder.block()
            return CityDictionary(builder.build().list)
        }
    }

}
